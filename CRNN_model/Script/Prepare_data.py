...
"""
    ...
    print("Grooming {:.2f} sec, Not Grooming: {:.2f} sec".format(grooming_time, non_grooming_time))
...
def get_raw_video_data(path,limit = 100):
    ...
    while cap.isOpened():
        ...
        (thresh, blackAndWhiteImage) = cv2.threshold(gray, 117, 255, cv2.THRESH_BINARY_INV)
...
def prepare_data(all_video, labels, sample_size = 2000):
    ...
    sample_idx = np.random.choice(np.arange(11, len(all_video) - 11), sample_size,
                                  replace=False)
    train_x = []
    train_y = []
    for i in sample_idx:
...
def prepare_data_31(all_video, labels, sample_size = 2000):
    ...
    # random sampling from all frames
    sample_idx = np.random.choice(np.arange(16, len(all_video) - 16), sample_size,
                                  replace=False)
    train_x = []
    train_y = []
    for i in sample_idx:
...
def half_half_sampling(all_video, labels, sample_size = 10800, time = 60, ratio = 1):
    ...
    ng_idx = np.random.choice(np.arange(11, len(non_grooming) - 11), n_non_grooming,
                                  replace=False)
    ...
    sample_idx = np.random.choice(np.arange(all_idx.shape[0]), sample_size,
                                  replace=False)
...
if __name__ == '__main__':
    ...
    train_x, train_y = half_half_sampling(all_video, labels, sample_size = 2000, time = 10)
