...
from tensorflow.keras.layers import Conv2D, BatchNormalization, MaxPool2D, GlobalMaxPool2D, TimeDistributed, GRU, Dense, Dropout, LSTM
...
def crnn_model(shape = (20, 112,112,3) , nbout = 2):
    ...
    model.add(TimeDistributed(
        tf.keras.layers.experimental.preprocessing.RandomRotation((-0.3,0.3))))
...
def plot_loss(train_loss, test_loss, train_acc, test_acc, fn = "loss_and_acc.png"):
    fig = plt.figure(figsize = (15,5))
    plt.subplot(121)
    plt.plot(train_loss)
    plt.plot(test_loss)
