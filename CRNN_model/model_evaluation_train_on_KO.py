...
def plot_result(pred, true, fn):
    ...
    plt.scatter( predict_g,np.full(len(predict_g), 0.05) , 
                marker = "|", s = 9000, )
    plt.scatter(true_g,np.zeros(len(true_g)), 
                marker = "|", s = 9000, color = 'red', )
    
    blue = mlines.Line2D([], [], color='blue', marker='|', linestyle='None',
                          markersize=10, label = "Predticted")
...
if __name__ == "__main__":
    ...
    for i in range(len(mouses)):
        ...
        test_x, test_y = prepare_data_31(all_videos, all_labels, sample_size = 900*8)
    ...
    all_mice_cm = pd.DataFrame(all_cm, index = mouses, columns = ["TP","FN", "FP", "TN"])
